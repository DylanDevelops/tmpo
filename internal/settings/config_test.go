...
func TestCreateWithTemplate(t *testing.T) {
	t.Run("creates templated config file", func(t *testing.T) {
		...
		err = CreateWithTemplate("templated-project", 99.99, "Test description", "/tmp/exports")
		assert.NoError(t, err)
		...
		assert.Contains(t, string(content), "export_path: \"/tmp/exports\"")
		...
		assert.Equal(t, "Test description", cfg.Description)
		assert.Equal(t, "/tmp/exports", cfg.ExportPath)
	})
